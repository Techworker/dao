...
class KycAction extends AbstractAction
{
    /**
    ...
    public function __invoke(Request $request, Contractor $contractor)
    {
        /** @var User $user */
        $user = \Auth::user();
        $contractor->setStatus(Contractor::STATUS_KYC, 'Started KYC');
        $request->session()->flash('flash', 'Added contractor to KYC processing.');
